...
MPDParser.prototype.parseInitializationFromRepresentation = function(representations, periods, groups, node) {
...
MPDParser.prototype.parseInitialization = function(representations, periods, groups, initNode) {
    ...
    for (var i = 1; i < attribs; i++) {
        ...
        if (mnode.hasAttribute(attribValue))
            eval("this.pmpd.period[periods].group[groups].representation[representations].initializationSegment."
                    + getKeyByIndex(MPD.initialization, i).toString()
                    + "= mnode.attributes.getNamedItem(attribValue).value");
        // console.log("pmpd.period["+periods+"].group["+groups+"].representation["+representations+"].initializationSegment."
        // + getKeyByIndex(MPD.initialization,i).toString() + "=" +
        // eval("this.pmpd.period[periods].groups[groups].representation[representations].initializationSegment."
        // + getKeyByIndex(MPD.initialization,i).toString()));
    }
};
...
MPDParser.prototype.parseSegmentList = function(representations, periods, groups, node) {
    ...
    for (var i = 1; i < attribs; i++) {
        attribValue = eval("MPD.segmentList."
                + getKeyByIndex(MPD.segmentList, i).toString());
        if (node.hasAttribute(attribValue))
            eval("this.pmpd.period[periods].group[groups].representation[representations].segmentList."
                    + getKeyByIndex(MPD.segmentList, i).toString()
                    + "= node.attributes.getNamedItem(attribValue).value");
        // console.log("pmpd.period["+periods+"].group["+groups+"].representation["+representations+"].segmentList."
        // + getKeyByIndex(MPD.segmentList,i).toString() + "=" +
        // eval("this.pmpd.period[periods].groups[groups].representation[representations].segmentList."
        // + getKeyByIndex(MPD.segmentList,i).toString()));
    }
    ...
    for (var s = 0; s < segmentListChilds.length; s++) {
            ...
            for (var i = 1; i < attribs; i++) {
                ...
                if (node.hasAttribute(attribValue))
                    eval("this.pmpd.period[periods].group[groups].representation[representations].segmentList.segment[segments]."
                            + getKeyByIndex(MPD.segmentURL, i).toString()
                            + "= node.attributes.getNamedItem(attribValue).value");
                // console.log("pmpd.period["+periods+"].group["+groups+"].representation["+representations+"].segmentList.segment["+segments+"]."
                // + getKeyByIndex(MPD.segmentURL,i).toString() + "=" +
                // eval("this.pmpd.period[periods].groups[groups].representation[representations].segmentList.segment[segments]."
                // + getKeyByIndex(MPD.segmentURL,i).toString()));
            }
            segments++;
        } else if (segmentListChilds.item(s).nodeName == MPD.initialization.name) {
...
MPDParser.prototype.parse = function() {
    if (this.mpd.documentElement.tagName == MPD.rootElement.name) {
        ...
        var periods = 0;
        for (var c = 0; c < childsFromRoot.length; c++) {
